...
//cria os vetores para produtos relacionados
var idsProds=[];
var descProds = [];
var precoProds = [];
var tamanhoProds =[];
...
function pesquisaProduto(){
    //limpar os produtos relacionados
    limpa_vetores();
    //requisita o backend
    ...
    //manipula os dados do json de resposta
    .then(data=>{
        ...
        breadcrumb.textContent = 'Home > '+categoria+
        ' > '+desc;
        fotoProd.setAttribute('src',
            './assets/images/products/'+foto);
        idProd.textContent = 'Cód: '+id;
        nomeProd.textContent = desc;
        tamanhoProd.innerHTML = '<div class="btn-icon">'+
            tamanho+'</div>';
        precoProd.textContent = 'R$: '+preco;
        descProd.innerHTML = desc+'<br>'+desc+'<br>'+desc;
        let precoNormal = preco * 1.5;
        precoCheio.innerHTML = 'de <span>R$ '
        +precoNormal.toFixed(2)+'</span> por';
